...
class PyInstArchive:
    ...
    def extractFiles(self, custom_dir=None):
        ...
        for each in self.tocList:
            ...
            if('/' in each.name or '\\' in each.name):
                continue
            if '' == fileExt:
                ...
                if(self.pyver <= 36):
                    ...
                    # subprocess.run(["decompyle3", each.name + ".pyc", ">", each.name + ".py"], stderr=subprocess.PIPE, shell=True)
                    p = Popen(["decompyle3", each.name + ".pyc", ">", each.name + ".py"], stderr=PIPE, shell=True)
                    output=p.stderr.read()
                    if "decompyle3' is not recognized" in str(output):

                        print("****************************************************************\n\n")
                        print(" [!] Please check that Python 3.8 is installed on this system")
                        print(" [!] Decompyle3 isn't installed - Please download from https://github.com/rocky/python-decompile3")
                        print(" [!] Move the extracted zip to your Documents directory")
                        print(" [!] And then run the following command once downloaded 'python3 pip install .' *NOTE THE FULL STOP")
                        print(" [!] Then rerun this program!")
                        exit(1)
                    else:
                        pass
